...
if 'django.contrib.contenttypes' not in settings.INSTALLED_APPS:
    raise ImportError("xratings requires django.contrib.contenttypes in "
                      "your INSTALLED_APPS")
...
class RatingManager(object):
    ...
    def get_avg_rating(self):
        if sum(self.scores) == 0:
            return 0
        return float(self.get_real_rating()) / sum(self.scores)
    ...
    def get_rating_for_user(self, user, ip_address=None, cookies={}):
        ...
        #TODO: IP address is not mandatory
    ...
    def add(self, score, user, ip_address, cookies=None):
        if not cookies:
            cookies = {}
        ...
        try:
            ...
            raise InvalidRating("%s is not a valid choice for %s, "
                                "type missmatch" % (score, self.field.name))

        delete = (score == 0)
        if delete and not self.field.allow_delete:
            raise CannotDeleteVote("you are not allowed to delete "
                                   "votes for %s" % (self.field.name,))

        if not self.field.check_range(score):
            raise InvalidRating("%s is not a valid choice for %s, score "
                                "not in range" % (score, self.field.name))
        ...
        try:
            ...
            if delete:
                raise CannotDeleteVote("attempt to find and delete your vote "
                                       "for %s is failed" % (self.field.name,))
        ...
        if not created:
            ...
            if not (isinstance(self.scores, list) and
                            len(self.scores) == len(
                                    [0] * len(self.field.range))):
    ...
    def _get_scores(self, default=None):
    ...
    scores = property(_get_scores, _set_scores)
